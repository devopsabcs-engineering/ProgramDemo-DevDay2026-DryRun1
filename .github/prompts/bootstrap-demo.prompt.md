...
2. `.github/prompts/*.prompt.md` — the 3 HVE-Core prompt files generated by this meta-prompt
...
|---------|-----|------|---------|
| 0–8 | Opening | Presenter | The Problem — show empty repo (only README.md + prompts), Azure portal (`rg-dev-125`), empty ADO board |
...
The prompt must state clearly: scaffolding is generated during the live demo, no ADO work items exist, only README.md and prompts exist at start, and `TALK-TRACK.md` goes at the repository root.
...
The 3 generated prompt files must be complete, self-contained, and paste-and-run. A user with only this `README.md` and the 3 prompts can run them in sequence and produce all 13 scaffolding files (7 configuration + 3 documentation + 2 scripts + TALK-TRACK.md) without further prompt engineering.
